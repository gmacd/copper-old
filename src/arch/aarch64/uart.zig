...
pub const Serial = struct {
    ...
    /// Initialise default serial port (UART0)
    pub fn init() Serial {
        return initUart(UART0_BASE);
    }

    /// Initialise with specific serial port
    pub fn initUart(uartBase: u32) Serial {
        return Serial{
            .uartBase = uartBase,
        };
    }
    ...
    pub fn initPreInterrupt(self: *const Serial) void {
        // Disable uart
        mmio.write(self.uartBase + UART_CR, 0x0);
        ...
        mmio.write(self.uartBase + UART_ICR, 0x7ff);
        ...
        mmio.write(self.uartBase + UART_IBRD, 1);
        // Fractional part register = (.627 * 64) + 0.5 = 40.6 = ~40.
        mmio.write(self.uartBase + UART_FBRD, 40);

        // Enable FIFO & 8 bit data transmission (1 stop bit, no parity).
        mmio.write(self.uartBase + UART_LCRH, (1 << 4) | (1 << 5) | (1 << 6));

        // Mask all interrupts.
        mmio.write(self.uartBase + UART_IMSC, (1 << 1) | (1 << 4) | (1 << 5) | (1 << 6) |
            (1 << 7) | (1 << 8) | (1 << 9) | (1 << 10));

        // Enable UART0, receive & transfer part of UART.
        mmio.write(self.uartBase + UART_CR, (1 << 0) | (1 << 8) | (1 << 9));
    }

    fn transmitterEmpty(self: *const Serial) bool {
        return mmio.read(self.uartBase + UART_FR) & (1 << 5) > 0;
    }

    pub fn printChar(self: *const Serial, c: u8) void {
        while (!self.transmitterEmpty()) {}
        mmio.write(self.uartBase + UART_DR, c);
    }
